...
export function Repositories(): JSX.Element {
  ...
  const searchRepositoryStatus = watch("searchRepositoryStatus");
  const isRepositoryLoading = watch("isRepositoryLoading");
  ...
  return (
    <RepositoryContainer>
      ...
      ) : searchRepositoryStatus?.length > 0 ? (
        <ErrorText>{searchRepositoryStatus}</ErrorText>
      ) : (
